...
const theme: ThemeConfig = {
	...
	init: () => {
		...
		if (s1) s1.style.boxShadow = genStarsBoxShadow(200, 1, 0.4, false)
		if (s2) s2.style.boxShadow = genStarsBoxShadow(100, 2, 0.6, false)
	},
	onMouseMove: (xPct, yPct) => {
		const bg = document.getElementById('tc-bg')
		const orbs = document.getElementById('tc-orbs')
		if (bg) bg.style.transform = `translate(${(xPct - 0.5) * 20}px, ${(yPct - 0.5) * 20}px)`
		if (orbs)
