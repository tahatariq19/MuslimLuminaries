  ...
  document.addEventListener("DOMContentLoaded", function () {
    ...
    function pickRandomStart() {
      ...
      if (shuffledQuotes.length > 1) {
        ...
        [shuffledQuotes[0], shuffledQuotes[randIdx]] = [
          shuffledQuotes[randIdx],
          shuffledQuotes[0],
        ];
      }
    }
    ...
    function unloadAssets(bgClass) {
      const assets = {
        "bg-nebula-particles": {
          css: "css/nebula_particles.css",
          js: "js/nebula_particles.js",
        },
        "bg-wavy-gradient": { css: "css/wavy_gradient.css", js: null },
        "bg-cosmic-particles": {
          css: "css/cosmic_particles.css",
          js: "js/cosmic_particles.js",
        },
      };
    ...
    async function applyBackground(idx) {
      ...
      if (
        bgClass === "bg-cosmic-particles" &&
        typeof createCosmicParticles !== "undefined"
      ) {
      ...
      if (
        bgClass === "bg-nebula-particles" &&
        typeof createNebulaParticles !== "undefined"
      ) {
      ...
      // Persist
      try {
        localStorage.setItem(BG_KEY, bgClass);
      } catch (e) {
        /* ignore */
      }
    }
    ...
    if (bgToggle) {
      ...
      // Make the shortcut safer by ignoring keypresses when the user is typing
      // in inputs, textareas, selects, or contenteditable elements.
      document.addEventListener("keydown", (ev) => {
        try {
          ...
          const isEditable =
            active &&
            (active.isContentEditable || /INPUT|TEXTAREA|SELECT/.test(tag));
